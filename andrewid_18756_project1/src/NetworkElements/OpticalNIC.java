...
public class OpticalNIC {
	...
	public void setInLink(OtoOLink inLink) {
		if (inLink.getSource() != this && inLink.getDest() == this)
			...
			System.err.println(
					"Error (OpticalNIC): You tried to assign an Optical in link that was not created for this interface");
	}
	...
	public void setOutLink(OtoOLink outLink) {
		if (outLink.getSource() == this && outLink.getDest() != this)
			...
			System.err.println(
					"Error (OpticalNIC): You tried to assign an Optical out link that was not created for this interface");
	}

	public OtoOLink getOutLink() {
		return this.outLink;
	}

	/**
	 * Sets the ID for this optical NIC. Each NIC in a router should have a unique
	 * ID so that it can
	...
	/**
	 * Set if this NIC goes in a clockwise direction. Could be one way of telling
	 * the shortest path since
	...
	/**
	 * Set of this NIC is on a SONET ring. If the NIC is part of the ring
	 * wavelengths will be forwarded on
	...
	/**
	 * Returns whether or no this NIC is part of a SONET ring. If it is part of a
	 * ring then all wavelengths
	...
	/**
	 * Returns the working NIC of this NIC, or null if this card is not a protection
	 * NIC
	 * 
	...
	/**
	 * Returns the protection NIC of this NIC, or null if this card does not have a
	 * protection NIC
	 * 
	...
	public void setHasError(Boolean hasError) {
		System.out.println(
				"(OpticalNIC) " + this.parent.getAddress() + " " + this.getID() + ": Set error state: " + hasError);
	...
	public Boolean sendPacket(Packet packet, int wavelength) {
		...
		if (this.outLink == null) {
			System.err.println(
					"Error (OpticalNIC): you are trying to send a frame on an interface with no out connection:");
			return false;
		}
		if (this.getHasError()) {
			System.err.println(
					"Error (OpticalNIC): you are trying to send a frame on an interface that is in an error state");
			return false;
		} else {
	...
	/**
	 * This method is run when a packet is received from the input link joined to
	 * this NIC. It is then
	...
	public void receiveData(Packet packet, int wavelength) {
		if (this.trace)
			System.out.println("Trace (OpticalNIC): router:" + this.parent.getAddress() + " NICid: " + this.getID()
					+ " received data");

		if (this.trace)
			System.out.println(
					"Trace (OpticalNIC): " + this.parent.getAddress() + " has received a Packet on " + wavelength
