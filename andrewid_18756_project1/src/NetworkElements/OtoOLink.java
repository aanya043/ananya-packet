...
public class OtoOLink {
	...
	/**
	 * Returns the source NIC for this link. i.e. the only NIC that can put data
	 * onto this link
	 * 
	...
	/**
	 * Returns the destination NIC for this link. i.e. the NIC where data put on
	 * this link will be delivered to
	 * 
	...
	/**
	 * Sets that this link has been cut. Making both ends senSe there have been a
	 * loss of light
	...
	/**
	 * Sends data from the source NIC joined to this link, to the dest NIC joined to
	 * this link
	 * 
	 ...
	 *                   (non-Javadoc)
	 * @see NetworkElements.OtoOLinkTA#sendData(DataTypes.STS3packet, int,
	 *      NetworkElements.OpticalNICTA)
	 */
	public void sendData(Packet packet, int wavelength, OpticalNIC source) {
		if (dest == null)
			...
			if (packet instanceof STS1Packet) {
				...
				((STS1Packet) packet).setNic(source);
